...
import { productStatus } from "@/helpers/roles";

const Products = ({ isWarning, handleShowWarning }) => {
  const { products, isLoading, currentPage, totalPages } = useSelector(
  ...
  const handlePageChange = (newPage) => {
    if (newPage !== currentPage) {
      dispatch(fetchProducts({ limit: 10, page: newPage }));
    }
  };
        ...
        >
          ...
          {openMenuId === row?._id && (
            <div className="custom-dropdown">
              <NavLink to={`/products/show/${row._id}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
  ...
  if (isLoading) {
    return (
      <div className={`page ${styles.products}`}>
        <Loading />
      </div>
    );
  }
          ...
          <Col xs={12} md={6}>
            <div className="text">
              ...
              <p className="page-description">
                <FormattedMessage id="productsDescription" />
              </p>
            </div>
          </Col>
          <Col xs={12} md={4}>
